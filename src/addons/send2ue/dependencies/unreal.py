...
class UnrealImportLevelSequence(Unreal):
    ...
    def import_cameras(self, level_sequence, cam_tracks, start_frame, end_frame):
        ...
        for camera_name, track in cam_tracks.items():
            ...
            fov_track.set_property_name_and_path('FieldOfView', 'FieldOfView')
            fov_section = fov_track.add_section()
            fov_section.set_start_frame_bounded(0)
            fov_section.set_end_frame_bounded(0)
            for key in track["fov"]:
                fov_section.get_all_channels()[0].add_key(time=unreal.FrameNumber(key[0]), new_value=key[1])

            # add camera cut section
            frame_range = track["frame_range"]
            camera_cut_section = camera_cut_track.add_section()
            camera_cut_section.set_range(*frame_range)
...
class UnrealRemoteCalls:
    ...
    def import_level_sequence(asset_data, property_data, start=None, end=None):
        ...
        unreal_import_level_sequence = UnrealImportLevelSequence(asset_data)
        # run the import task
